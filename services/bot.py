...
GEOCODE = "https://nominatim.openstreetmap.org/search"  # requires User-Agent

# session with retries + UA (Nominatim requires UA)
...
def geocode_place(q: str) -> tuple[float, float] | None:
    try:
        r = _session.get(GEOCODE, params={"q": q, "format": "json", "limit": 1}, timeout=8)
        r.raise_for_status()
        arr = r.json()
        if not arr:
            return None
...
def otm_search_in_state(state: str, limit: int = 15) -> list[dict]:
    """Find attractions around a state's centroid via geocode + OpenTripMap."""
...
def places_for_state(state: str) -> dict:
    ...
    if not rows:
        return {
            "message": f"I couldn’t fetch attractions for {state.title()} right now.",
...
def details_for_place(name: str) -> dict:
    info = wiki_enrich(name) or {}
    lat = lon = None
    if info.get("coords"):
        lat = info["coords"].get("lat")
        lon = info["coords"].get("lon")
    ...
    chunks = []
    if info.get("summary"):
        chunks.append(info["summary"])
    if wx:
        chunks.append(wx)
    chunks.append(
        f"Maps: https://www.google.com/maps/search/?api=1&query={requests.utils.quote(name)}"
    )

    return {
        "message": "\n\n".join(chunks) or f"{name} is a notable destination.",
        "suggestions": [f"Best time to visit {name}", f"Things to do in {name}"],
    }


def chat_reply(message: str, session_id: str = "") -> dict:
    """Very light router: if it's a state, list places; else, place details."""
    try:
        s = is_state(message)
        if s:
            return places_for_state(s)
        m = re.sub(
            r"^(tell me about|info on|details about)\s+",
            "",
            (message or "").strip(),
            flags=re.I,
        )
        return details_for_place(m or message)
    except Exception as e:
        # Always return a JSON the UI can show
        return {"message": "Sorry, I hit an error.", "suggestions": [], "error": str(e)}
